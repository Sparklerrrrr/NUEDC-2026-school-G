...
// @return		void
// Sample usage:	pwm_pin_init(timn_chn);  内部使用 勿外部调用
//-------------------------------------------------------------------------------------------------------------------
...
void pwm_init(TIMn_enum timn,TIMn_CHn_enum timn_chn,int fre)
{
  ...
  pwm_pin_init(timn_chn);
	...
	tim_index[timn]->CR1 |= 0x81;        //使能计数器和中断ARR
}
...
void pwm_update(TIMn_enum timn,TIMn_CHn_enum timn_chn,uint16_t duty)
{
	...
	if(duty>=MAX_DUTY)
		duty=MAX_DUTY;
	uint16_t temp = tim_index[timn]->ARR;   //获取ARR
	switch(ch)
	{
		case 1:
